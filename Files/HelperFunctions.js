async function getneighbors(current_state) {
...
async function getNull(my_state) {
  let row;
  let col;
  for (row = 0; row < total_rows; row++) {
...
async function checkSameState(state1, state2) {
...
async function getColumns(current_state) {
  let all_col = [];
  let temp_col = [];
